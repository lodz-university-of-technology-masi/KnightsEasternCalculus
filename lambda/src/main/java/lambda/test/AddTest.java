    ...
    public Response handleRequest(Test input, Context context) {
        if (input != null) {
            if (input.getId() == null) {
                ...
                return new Response(200, "Test added succesfully");
            } else {
                if (getMapper().load(Test.class, input.getId()) == null) {
                    getMapper().save(input);
                    return new Response(200, "Test added succesfully");
                } else return new Response(500, "Test did not added (such test exists)");
            }
        } else return new Response(500, "Test did not added");
    }
